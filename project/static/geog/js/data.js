...
const fetchGeoJSONDataMap = new Map()
const fetchGeoJSONData = async (event, layer, options={}) => {
    const defaultTooltip = `Zoom in to load individual ${layer.data.layerTitle} features.`
    ...
    const geojson = (async () => {
        try {
            ...
            if (!geojson) {
                geojson = await fetchLibraryData(event, layer)
                if (!geojson) {
                    geojson = {
                        type: 'FeatureCollection',
                        features: [turf.polygonToLine(turf.bboxPolygon(layer.data.layerBbox.slice(1, -1).split(',')))],
            ...
            if (cacheKey && geojson.cachedGeoJSON) {
    ...
    if (cacheKey) {
        fetchGeoJSONDataMap.set(cacheKey, geojson)
    }
