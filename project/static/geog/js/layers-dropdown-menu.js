const populateLayerDropdownMenu = async (toggle, options={}) => {
    ...
    const geojson = options.geojson || (await getFromGeoJSONDB(getLayerKey(currentLayer))) || layerToGeoJSON(currentLayer)
    ...
    const duplicateBtn = !currentCheckbox ? createDropdownMenuListItem({
        ...
        buttonClickHandler: () => toggleLayer({target:duplicateBtn.querySelector('button')}, {
            map: map,
