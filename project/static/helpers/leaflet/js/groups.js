const handleLeafletLayerGroups = (map) => {
    ...
    Array('library', 'client', 'query').forEach(groupName => {
        ...
        group._ch = {
            ...
            getHiddenLayer: (id) => {
                const layer = group._ch.getHiddenLayers().find(l => l._leaflet_id === id)
            ...
            findLayer: (id) => {
                const layer = group.getLayer(id) ?? group._ch.getHiddenLayer(id) ?? group._ch.getInvisibleLayer(id) 
                console.log(id, group._hiddenLayers, layer)
                return layer
            },
