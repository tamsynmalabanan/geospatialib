...
return(compressedExtensions.includes(file.name.toLowerCase().split('.').pop())||compressedExtensions.some(i=>(file.type??'').includes(i)))}
...
return geojson}}).catch(error=>{console.log(error)})}
const fetchSHP=async(params,{abortBtns,controller}={})=>{return await fetchTimeout(params.url,{abortBtns,controller,callback:async(response)=>{const buffer=await response.arrayBuffer()
const geojson=await shp({shp:buffer})
return geojson}}).catch(error=>{console.log(error)})}
const rawDataToLayerData=async(rawData,params,{filesArray=[]})=>{try{if(params.type==='geojson'){return JSON.parse(rawData)}
...
if(params.type==='shp'){const name=params.name.split('.shp')[0]
const shpFiles={}
for(const ext of['shp','dbf','prj','cpg']){const file=filesArray.find(file=>file.name===`${name}.${ext}`)
if(!file)continue
...
const fetchFileData=async(params,{abortBtns,controller}={})=>{const handler=async(filesArray)=>{const file=(filesArray.length===1?filesArray[0]:filesArray.find(file=>file.name===params.name))
if(!file)return
...
if(geomType){try{const[x,y]=geomType==='Point'?feature.geometry.coordinates:turf.centroid(feature).geometry.coordinates
...
return table}
const fetchGeoJSONHandlers=(name)=>{return{osm:fetchOSMData,nominatim:fetchNominatim,overpass:fetchOverpass,geojson:fetchGeoJSON,file:fetchFileData,csv:fetchCSV,gpx:fetchGPX,kml:fetchKML,shp:fetchSHP,'ogc-wms':fetchWMSData,'ogc-wfs':fetchWFSData,}[name]}
const staticVectorFormats=['local','file','geojson','csv','gpx','kml','shp','osm',]
...
geojsonLayer.on('add',()=>updateLeafletGeoJSONLayer(geojsonLayer,{geojson:(geojsonLayer.getLayers().length&&geojsonLayer!==geojsonLayer._group?._map?._drawControl?._targetLayer&&geojsonLayer._previousVersion===geojsonLayer._indexedDBKey&&previousBboxGeom&&turf.booleanEqual(geojsonLayer._previousBbox.geometry,previousBboxGeom)?geojsonLayer.toGeoJSON():null),updateLocalStorage:false}))
...
let layer
if(Array(format,type).some(i=>Array('geojson','csv','gpx','kml','shp','wfs','osm','overpass','unknown','json').includes(i))){layer=await getLeafletGeoJSONLayer({indexedDBKey,geojson:data,group,pane,params,properties,customStyleParams,})}else{if(type==='xyz'){layer=L.tileLayer(params.url,{pane,maxZoom:20,})}
...
localStorage.removeItem(i)})}})};const COLLECTION_FORMATS={'overpass':'Overpass API','osm':'OpenStreetMap','geojson':'GeoJSON','csv':'CSV','gpx':'GPX','kml':'KML','shp':'Shapefile','file':'File','xyz':'XYZ Tiles','ogc-wfs':'OGC Web Feature Service','ogc-wms':'OGC Web Map Service',}
