...
const zoomToSearchResultBbox=()=>{const layerParams=event.target.parentElement.dataset.layerParams
return getSearchMap()?.fitBounds(L.geoJSON(turf.bboxPolygon(JSON.parse(layerParams).bbox)).getBounds())}
const showLayerInfo=async(el)=>{const modal=document.querySelector(el.dataset.bsTarget)
...
name.parentElement.firstElementChild.innerText=(Array('wms','wfs').includes(properties.type)?'Layer':Array('overpass').includes(properties.type)?'Tag':'Name')
...
if(properties.type==='overpass'){const width=previewThumbnails.offsetWidth
...
properties.thumbnails.forEach(i=>{carouselItems.appendChild(customCreateElement({className:`carousel-item ${properties.thumbnails[0] === i ? 'active': ''}`,innerHTML:`<img src="${i}"} width="${width}" height="${width/360*180}">`,}))})}else{const bboxGeoJSON=turf.bboxPolygon(properties.bbox)
if(Array('wfs').includes(properties.type)){previewLabel.innerText='Bounding Box Map'
const bboxLayer=await getLeafletGeoJSONLayer({geojson:bboxGeoJSON,group,customStyleParams:{strokeColor:'hsla(0, 100%, 54%, 1.00)',fillOpacity:0,strokeWidth:3},})
group.addLayer(bboxLayer)}else{previewLabel.innerText='Layer Map'
addLayerFromData(footer,{map:previewMap,customStyleParams:{fillColor:'hsla(0, 100%, 54%, 1.00)',strokeWidth:3}})}
setTimeout(()=>{previewMap.fitBounds(L.geoJSON(bboxGeoJSON).getBounds())},500)}
...
const infoSource=(()=>{let url
if(properties.type==='overpass'){if(Array('=','~').some(i=>properties.tags.includes(i))){let[key,value]=properties.tags.split(/[=~]/,2)
key=key.split('"')[1]
value=value.split('"')[1]
url=`https://taginfo.openstreetmap.org/api/4/tag/overview?key=${key}&value=${value}`}else{let key=properties.tags.split('"')[1]
url=`https://taginfo.openstreetmap.org/api/4/key/overview?key=${key}`}}
if(properties.format.startsWith('ogc-')){url=`${properties.url}?service=${properties.type.toUpperCase()}&request=GetCapabilities`}
...
attribution.innerHTML=(Array('',null,undefined,'none').includes((properties.attribution??'').trim().toLowerCase())?`Retreived from <a target="_blank" href="${properties.url}">${hostname}</a>, hosted by <a target="_blank" href="https://${domain}">${domain}</a>`:properties.attribution)
...
styles.innerHTML=Object.keys(stylesObj).map(i=>{return`Name: ${i}<br>Title: ${stylesObj[i].title}<br>Legend:<br><img src="${stylesObj[i].legend}">`}).join('<br>')}
const addSearchResultBboxToMap=async(el)=>{const addBtn=el.previousElementSibling
...
const customStyleParams={strokeColor:strokeColor.toString(),strokeOpacity:1,strokeWidth:3,dashArray:`3 6`,dashOffset:strokeColor.h}
const layer=await getLeafletGeoJSONLayer({geojson:turf.polygonToLine(turf.bboxPolygon(properties.bbox,{properties})),pane:'searchPane',group,customStyleParams,params:{type:'geojson',name:'search result'}})
if(layer){layer._addBtn=addBtn
group?.addLayer(layer)}}
const toggleSearchResultBbox=async(event)=>{const searchResults=document.querySelector('#searchResults')
const map=getSearchMap()
const group=map?._handlers.getLayerGroups().search
if(group?.getLayers().length){group.clearLayers()
map.getPane('searchPane').innerHTML=''}else{for(i of Array.from(searchResults.querySelectorAll(`[onclick="zoomToSearchResultBbox()"]`))){await addSearchResultBboxToMap(i)}}
...
document.addEventListener('DOMContentLoaded',()=>{const form=getSearchForm()
const queryField=form.elements.query
form.addEventListener('submit',(e)=>e.preventDefault())
form.addEventListener('htmx:configRequest',(e)=>{const requestParams=e.detail.parameters
if(!Object.keys(requestParams).includes('clear')){const urlParams=Object.fromEntries(new URLSearchParams(window.location.search))
for(const key in urlParams){if(Object.keys(requestParams).includes(key))continue
if(!urlParams[key])continue
requestParams[key]=urlParams[key]}}
Object.keys(requestParams).forEach(key=>{if(requestParams.get(key))return
requestParams.delete(key)})})
form.addEventListener('htmx:beforeRequest',(e)=>{if(!queryField.value)return e.preventDefault()
...
document.querySelector('#searchResults').innerHTML=removeWhitespace(`
        ...
        `)
const map=getSearchMap()
if(map){map._handlers.getLayerGroups().search.clearLayers()
map.getPane('searchPane').innerHTML=''}
const urlParams=e.detail.pathInfo.finalRequestPath.split('?')
window.history.pushState({},'',`${window.location.pathname}?${urlParams[urlParams.length-1]}`)})
form.addEventListener('htmx:afterSwap',(e)=>{const map=getSearchMap()
if(map)Array.from(form.querySelectorAll(`[data-map-bbox-field="true"]`)).forEach(i=>{i.value=JSON.stringify(turf.bboxPolygon(getLeafletMapBbox(map)).geometry)})})
document.addEventListener('htmx:responseError',(e)=>{if(e.detail.requestConfig.path.startsWith('/htmx/library/search/')){const target=e.detail.target
...
searchResults.parentElement.addEventListener('htmx:afterSwap',(e)=>{if(e.target.id===searchResults.id)return
...
if(queryField.value){const submitEvent=new Event('submit')
form.dispatchEvent(submitEvent)
const offcanvas=form.closest('.offcanvas')
if(offcanvas)bootstrap.Offcanvas.getOrCreateInstance(offcanvas).show()}})
document.addEventListener('DOMContentLoaded',()=>{const layerInfoModal=document.querySelector(`#layerInfoModal`)
layerInfoModal.addEventListener('shown.bs.modal',(e)=>{showLayerInfo(e.relatedTarget)})});