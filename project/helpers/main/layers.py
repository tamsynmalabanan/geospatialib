...
def csv_to_geojson(file, params):
    try:
        ...
        xField = params['xField'] = params.get('xField', ([i for i in df.columns if any([j for j in LONGITUDE_ALIASES if j in i.strip().lower()])]+[None])[0])
        yField = params['yField'] = params.get('yField', ([i for i in df.columns if any([j for j in LATITUDE_ALIASES if j in i.strip().lower()])]+[None])[0])
